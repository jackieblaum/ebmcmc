...
class EBMCMC:
    ...
    def __init__(self, bundle, trace_dir=None, sed=None, datasets=None, eclipsing=True, ecc=True):
        """
    ...
    def initialize_logging(self):
        ...
        phoebe_logger = phoebe.logger(clevel=None, flevel='CRITICAL', filename='phoebe.log')
    ...
    def extract_light_curve_data(self, dataset):
        ...
        self.min_time, self.max_time = min(self.min_time, np.min(times)), max(self.max_time, np.max(times))
        return {
    ...
    def define_model(self):
        ...
        pblums_init = [self.bundle.get_value(f'pblum@primary@{dataset}@dataset') for dataset in self.bundle.datasets if dataset.startswith('lc')]
        ...
        with pm.Model() as self.model:
            ...
            mass_primary = 39.478418 * (asini / tt.sin(incl * (2 * np.pi) / 360)) ** 3 / (period ** 2 * (q + 1))
            ...
            fit_params = [
                teffratio, incl, requivsumfrac, requiv_secondary, q,
                period, sigma_lnf, teff_secondary
            ]
    ...
    def sample(self, ndraws=1000, cores=256, tune_steps=1000, continue_sampling=True):
        ...
        with self.model:
            trace = pm.sample(draws=ndraws, cores=cores, tune=tune_steps, progressbar=True)
    ...
    def load_full_trace_states(self, truncate=False):
        ...
        subdirs = [d for d in os.listdir(self.trace_dir) if os.path.isdir(os.path.join(self.trace_dir, d))]
        sorted_dirs = sorted(subdirs, key=lambda x: os.path.getctime(os.path.join(self.trace_dir, x)))
        ...
        for subdir in sorted_dirs:
            ...
            for trace_file in os.listdir(os.path.join(self.trace_dir, subdir)):
                if trace_file.endswith('.nc'):
                    ...
                    single_chain = az.from_netcdf(os.path.join(self.trace_dir, subdir, trace_file))
        ...
        total_draws = np.array([chain.sizes['draw'] for chain_set in all_chains for chain in chain_set])
        if len(set(total_draws)) > 1:
            inference_data_list = [az.InferenceData(posterior=chain) for chain in all_chains[-1]]
            if truncate:
                print(f'Chains have different lengths. Truncating to shortest chain length.')
                return self.truncate_chains(inference_data_list)
            print(f'Chains have different lengths. Returning individual traces for each chain in the most recent directory.')
            return inference_data_list

        # Combine chains into a single InferenceData object with multiple chains
        if len(all_chains) > 1:
            combined_chains = [xr.concat(chain_set, dim='draw') for chain_set in zip(*all_chains)]
            inference_data_list = [az.InferenceData(posterior=chain.expand_dims("chain")) for chain in combined_chains]
            combined_inference_data = az.concat(inference_data_list, dim="chain")
            return combined_inference_data
        else:
            inference_data_list = [az.InferenceData(posterior=chain) for chain in all_chains[0]]
            combined_inference_data = az.concat(inference_data_list, dim="chain")
            return combined_inference_data
    ...
    def posterior_predictive_checks(self, trace):
        ...
        for idx in range(num_samples):
            ...
            for param in posterior.data_vars:
                param_value = posterior[param].sel(chain=chain_idx, draw=draw_idx).values
                if param != 'sigma_lnf':
            ...
            if 'sed' in self.data_dict:
                ...
                fig, ax = sed_obj.plot_sed_and_model(
                    teff_primary, teff_secondary, requiv_primary, requiv_secondary, logg1, logg2, fig=fig, ax=ax)
